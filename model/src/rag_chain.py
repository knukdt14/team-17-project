...
- LLM은 Upstage Solar / Hugging Face / Google Gemini / Groq(Llama, Qwen)를 지원
"""
...
def get_llm(llm_key: str = "gemini"):
    ...
    raise ValueError(
        ...
        f"(가능한 값: 'solar', 'hf_open', 'gemini', {', '.join(repr(k) for k in GROQ_MODELS)})"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        for llm_key in ("solar", "hf_open", "gemini", "groq_llama", "groq_qwen"):
            print(f"\n================== [{llm_key}] ==================")
            try:
